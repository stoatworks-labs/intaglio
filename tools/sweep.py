...
## Why the context table is empty

Elsewhere in the fleet this script carries a table of extra settings, because
several controls are *supposed* to do nothing in the default configuration and
a naive sweep reports them as dead. Here every control is alive on the card as
it stands, and that was checked by emptying the table rather than assumed: a
run with no context at all reports all 25 live.

It is not luck. It is the card and the defaults, both built for it: the card
carries a flat panel with no edges in it, a tone ramp through every weight the
curve can ask for, rings, a disc and two fields of equal luminance, so every
threshold in the model is crossed somewhere on it; and the defaults ship with
two hatch sets rather than one, so `Cross Angle` has something to turn.

The table stays because the next control added may well need it. An entry
whose absence changes nothing is worse than no entry: it makes the sweep look
...
# Applied to every render. Nothing needed: the defaults already engrave the
...
def render(harness, out, settings, raw, verbose):
    ...
    if result.returncode != 0:
        raise RuntimeError(f"igtest failed: {result.stderr.strip()}")
...
def parameters(harness):
    ...
    if result.returncode != 0:
        raise RuntimeError(f"igtest --list failed: {result.stderr.strip()}")
...
def main():
    ...
    harness = REPO / args.build / "igtest"
    if not harness.exists():
        print(f"no igtest at {harness} -- build first", file=sys.stderr)
        return 2
