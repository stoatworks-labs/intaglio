...
# own grep that always runs. `patch` in particular is a natural name in a
# plugin about ruled lines and is reserved for tessellation; `sample`, `filter`
# and `layout` are just as easy to reach for.
#
...
# Mirrors PhaseShaderSource()/FlowProbeShaderSource().
ASSEMBLED = {
	"PhaseShader":     [ "#version 410 core\n", "kFlowLibrary", "kPhaseMain" ],
...
step "Bundle"
#---------------------------------------------------------------------------
bundle="$BUILD/Intaglio.bundle"
binary="$bundle/Contents/MacOS/Intaglio"
...
echo "ok   Intaglio: $arches, plugMain exported"
...
[[ "$identifier" == "com.stoatworks.ffgl.intaglio" ]] || fail "bundle id is '$identifier'"
[[ "$executable" == "Intaglio" ]] || fail "CFBundleExecutable is '$executable'"
...
if [[ -x "$OXBOW" ]]; then
	...
	printf '%s\n' "$probe" | sed 's/^/   /'
	grep -q 'IG01' <<<"$probe" || fail "oxbow did not read the id IG01"
	grep -q 'SW Intaglio' <<<"$probe" || fail "oxbow did not read the name 'SW Intaglio'"
	grep -qi 'effect' <<<"$probe" || fail "oxbow did not read the type as an effect"
	echo "ok   a host reads IG01 / SW Intaglio / effect"
else
...
"$BUILD/igtest" --weight
"$BUILD/igtest" --limits
"$BUILD/igtest" --perpendicular
"$BUILD/igtest" --crosshatch
